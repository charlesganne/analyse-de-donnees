...
cache: true # réutiliser des résultats déjà calculés (plus rapide quand on recompile)
message: false
warning: false # messages/warnings cachés dans le livrable
---
...
Enfin, il est étonnant de voir que le masse grasse n'est pas corrélé au poids ou à l'IMC. Cette variable n'augmente donc pas mécaniquement avec le poids, surtout dans une population sportive où la masse musculaire peut représenter une part importante du poids total. Cette observation avait déjà été faire lors de l'expérience documentée dans le film "super-size me" de Morgan Spurlock sorti en 2004. Le sujet n'avait pas prit beaucoup de poids mais était quand même devenu en très mauvaise santé, notamment à cause de sa masse grasse (et beaucoup d'autres troubles liés à la junk-food).
...
## Analyse de la troisième composante principale
...
L'analyse comparative met en évidence une forte cohérence entre les deux méthodes, avec un taux de concordance global de **`r round(accord, 1)`%**. Cette stabilité des résultats confirme la pertinence des groupes obtenus. Néanmoins, une meilleure efficacité pratique des K-means par rapport à la CAH apparaît clairement à travers l'analyse conjointe de la matrice de confusion (@tbl-confusion) et de la projection factorielle (@fig-kmeans-pca).
...
1.  **Le Cluster "Remise en forme"** (Correspondant au groupe avec *weight* \> 100kg dans @tbl-moyennes) :
    ...
    -   Majoritairement composé d'hommes d'après la @fig-comp de niveau débutant ou intermédiaire. Leurs séances sont plus courtes (\~1.1h).
...
3.  **Le Cluster "Fitness / Entretien"** (Correspondant au groupe avec *weight* \~ 60kg dans @tbl-moyennes) :
    ...
    -   Majoritairement féminin d'après la @fig-comp, mélangeant débutants et intermédiaires. C'est le groupe le plus volumineux, représentant une pratique d'entretien régulière mais modérée.
