...
```{r}
#| echo: true
# Récupération des coordonnées des individus sur les 3 premiers axes (F1, F2, F3)
# On retient 3 axes car nous avons vu que le 3ème (Taille) portait une information significative.
...
# 1. Calcul de la matrice de distance sur les coordonnées ACP
dist_acp <- dist(donnees_acp, method = "euclidean")

# 2. Application de la CAH (Méthode de Ward)
res_cah <- hclust(dist_acp, method = "ward.D2")

# 3. Visualisation
fviz_dend(res_cah, 
          k = 3, # Choix de 3 classes (cohérent avec l'ACP et les profils attendus)
          cex = 0.5, 
...
L'observation du dendrogramme suggère un découpage naturel en **3 classes**, correspondant à une perte d'inertie significative (saut important dans la hauteur des branches). Ce choix semble cohérent avec la structure attendue (débutant, intermédiaire, expert) ou les profils morphologiques identifiés lors de l'ACP.
...
```{r}
# --- CORRECTION DE L'ASSIGNATION DES CLUSTERS ---

# 1. Récupération sécurisée des clusters CAH
# On s'assure que 'grp_cah' (ou classes_cah) est bien un vecteur nommé
# (Note: assurez-vous d'avoir utilisé cutree() juste avant)
classes_cah <- cutree(res_cah, k = 3)

# 2. Initialisation des colonnes dans le tableau principal
SalleDeSport$cluster_cah <- NA
SalleDeSport$cluster_kmeans <- NA

# 3. Assignation par correspondance des noms de lignes (ROBUSTE)
# Cela permet d'éviter l'erreur de longueur si des lignes ont sauté
...
# --- MAINTENANT LA TABLE DE CONFUSION FONCTIONNERA ---

# On ne garde que les lignes où les deux clusters sont définis (pas de NA)
table_comp <- table(
...
# Calcul de l'indice de Rand (optionnel)
# library(mclust)
# adjustedRandIndex(SalleDeSport$cluster_cah, SalleDeSport$cluster_kmeans)
```
...
p2 <- ggplot(SalleDeSport, aes(x = cluster_kmeans, fill = gender)) +
  ...
  theme_minimal() +
  # On laisse ggplot choisir les couleurs de base pour éviter le conflit de noms,
  # ou on utilise une palette standard :
...
Ce dernier graphique permet de valider si un cluster spécifique regroupe majoritairement les experts ou s'il est mixte, et de voir si les différences physiologiques (Homme/Femme) ont créé des classes distinctes (ce qui est probable vu l'axe 2 de l'ACP).
