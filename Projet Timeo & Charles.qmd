...
library(FactoMineR)
library(factoextra)
# juste pour le fun pour tracer l'acp en 3d, mais à supprimer pour le rendu finale 
library(plotly)
```
...
#| echo: false #(#| echo: true) : inclure le code (ou pas)
...
-   *duration* : Durée de chaque séance d'entraî̂nement en heures
-   *calories* : Total des calories brûlées au cours de chaque séance
-   *fat* : Pourcentage de graisse corporelle du membre
-   *water* : Consommation quotidienne d'eau pendant les séances d'entraî̂nement
...
L'analyse en composantes principales est une méthode qui consiste à transformer des variables liées entre elles (corrélées) en nouvelles variables décorrélées les unes des autres. Ces nouvelles variables sont nommées "composantes principales" ou axes principaux. Elle permet de résumer l'information en réduisant le nombre de variables. 

Dans l'ensemble, les variables étudiées décrivent : la morphologie (poids, taille, IMC, masse grasse) ainsi que l'intensité et la pratique sportive (durée, calories brûlées, niveau, eau consommée). On a déjà vu par exemple que la durée de la séance et les calories dépensées étaient fortement corrélé. Idem pour le poids et l'IMC (ces deux données sont d'ailleurs redondantes car l'IMC est calculé en fonction du poid et de la taille). Les variables n’ont pas la même unité (kg, %, heures, calories, ...). On fait donc une ACP centrée réduite, sinon les calories ou le poids domineraient toutes les autres variables.
...
Les deux premières composantes principales expliquent 41.5% + 26.8% = 68,3 % de la variance totale, ce qui permet déjà une représentation satisfaisante des données. La troisième composante explique néanmoins 17,6 % de la variance. Elle n'est donc pas complètement négligeable. Ainsi, nous nous concentrerons principalement sur le premier plan factoriel (axes 1 et 2), tout en gardant à l'esprit l'apport potentiel de la troisième composante.
...
Dans un premier temps, on remarque que les flèches des variables weight, duration, calories, water, bmi sont longues Ces variables sont donc bien représentées sur le plan factoriel. La flèche de height est plus proche du centre, elle est donc moins bien représentée par les deux premières composantes principales.

L'axe des abscisse (Dim1, 41.5 %) est corrélé plus fortement à calories (0.80), duration (0.73), water (0.73) et fat (-0.83). Ainsi, cette composante principale rassemble les variables durée, calories et water, qui traduisent de l'intensité d'un entrainement, et les oppose à la variable masse grasse. C'est une observation que nous avions faite précédemment, les personnes avec une masse grasse plus élevées font des séances moins intenses.
...
#############################################
# graphique en 3d pour le fun (et pour preshot le clustering)

coords <- as.data.frame(res_pca$ind$coord[, 1:3])
coords$gender <- SalleDeSport$gender
plot_ly(
  coords,
  x = ~Dim.1,
  y = ~Dim.2,
...
5️⃣ Comment interpréter (exemples concrets)
Axe 1 (exemple)

L’axe 1 oppose des individus à forte intensité sportive (durée élevée, calories brûlées importantes, niveau élevé) à des individus moins actifs.

Axe 2 (exemple)

L’axe 2 est principalement lié à la morphologie corporelle, opposant des profils à IMC et masse grasse élevés à des profils plus secs.

Variables qualitatives

Les hommes apparaissent majoritairement du côté des valeurs élevées de calories brûlées, tandis que les femmes sont davantage associées à des pourcentages de masse grasse plus élevés.

(adapte selon TES résultats)

6️⃣ ACP à la main ou avec FactoMineR ?
✅ Pour le rapport

👉 FactoMineR uniquement
...
ou en annexe pédagogique (rare).

Phrase parfaite :

Les calculs sont réalisés à l’aide de la librairie FactoMineR, l’implémentation manuelle de l’ACP n’étant pas nécessaire dans un cadre d’analyse exploratoire appliquée.
